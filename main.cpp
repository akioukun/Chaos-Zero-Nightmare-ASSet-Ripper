...
bool is_valid_filemap(const nlohmann::ordered_json &j)
{
    if (!j.is_object())
        return false;
    ...
    const std::string gen = j["generator"].get<std::string>();
    if (gen != "Chaos-Zero-Nightmare-ASSet-Ripper")
    {
        return false;
    }
...
void export_to_json()
{
    try
    {
        ...
        if (!save_path.empty())
        {
            ...
            if (out.is_open())
            {
                nlohmann::ordered_json tree = build_file_tree_json(g_state.browser.data_pack->GetFileTree());
                nlohmann::ordered_json j;
...
int main(int argc, char *argv[])
{
    ...
    while (running)
    {
        ...
        if (g_state.common.show_error_popup)
        {
            ...
            const float popup_height = 140.0f;
            const float popup_x = (window_width - popup_width) * 0.5f;
            const float popup_y = (window_height - popup_height) * 0.5f;
            if (nk_begin(ctx, "Error", nk_rect(popup_x, popup_y, popup_width, popup_height), NK_WINDOW_BORDER | NK_WINDOW_MOVABLE | NK_WINDOW_TITLE))
            {
        ...
        if (nk_begin(ctx, "Main", nk_rect(0, 0, (float)window_width, (float)window_height), NK_WINDOW_NO_SCROLLBAR))
        {
            ...
            if (g_state.tasks.running)
            {
                ...
                nk_size prog_val = static_cast<nk_size>(cur_progress * 1000.0f);
